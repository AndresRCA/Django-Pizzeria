...
def dummyOrders(apps, schema_editor):
	...
	# models where data is inserted
	Order = apps.get_model('pizzeria', 'Order')
	Pizza = apps.get_model('pizzeria', 'Pizza')
	
	# logic: create order -> create pizza (related to order) -> generate toppings -> insert toppings through ToppingAmount model
	for i in range(10):
